...
public class ChilliPopClusterCheck implements ClusterCheck {
        ...
        for (int i = 0; i < matrix.length; i++)
            for (int j = 0; j < matrix[i].length; j++) {
                ...
                if (symbolId >=0 && symbolId <= 7) {
                    matrix[i][j] = -1;
                    recursion(matrix, symbolId, 8, countOfSymbols, countOfWild, i, j);
                    resetMatrix(matrix, 8);
    ...
    private void recursion(int[][] matrix, int symbolId, int wildId, int[] countOfSymbols, int[] countOfWild, int x, int y) {
        countOfSymbols[0]++;
        for (int i = x - 1; i <= x + 1; i++) {
            ...
            for (int j = y - 1; j <= y + 1; j++) {
                ...
                if (matrix[i][j] == symbolId || matrix[i][j] == wildId) {
                    if (matrix[i][j] != wildId) matrix[i][j] = -1;
                    else {
                        countOfWild[0]++;
                        matrix[i][j] = -2;
                    }
                    recursion(matrix, symbolId, wildId, countOfSymbols, countOfWild, i, j);
                }
            }
        }
    }

    private void resetMatrix(int[][] matrix, int wildId) {
        for (int i = 0; i < matrix.length; i++)
            for (int j = 0; j < matrix[i].length; j++)
                if (matrix[i][j] == -2) matrix[i][j] = wildId;
    }
}